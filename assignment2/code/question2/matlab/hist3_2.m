...
if nargs < 1
    error(message('stats:hist3:TooFewInputs'))
end
x = args{1};
...
if (isempty(nbins)+isempty(ctrs)+isempty(edges)+isempty(binSpec)) < 3
    error(message('stats:hist3:AmbiguousBinSpec'));
...
if ~isempty(nbins)
    ...
    if ~(isnumeric(nbins) && numel(nbins)==2)
        error(message('stats:hist3:BadNbins'));
    end
    autobins = true;
...
elseif ~isempty(ctrs)
    ...
    if ~(iscell(ctrs) && numel(ctrs)==2 && isnumeric(ctrs{1}) && isnumeric(ctrs{2}))
        error(message('stats:hist3:BadCtrs'));
    end
    ctrs = {ctrs{1}(:)' ctrs{2}(:)'};
    autobins = false;
...
elseif ~isempty(edges)
    ...
    if ~(iscell(edges) && numel(edges)==2 && isnumeric(edges{1}) && isnumeric(edges{2}))
        error(message('stats:hist3:BadEdges'));
    end
...
[nrows,ncols] = size(x);
if ncols ~= 2
    error(message('stats:hist3:WrongNumCols'));
end
            ...
            if nargout > 1
                ...
                if any(c <= e(1:end-1)) || ...
                   abs(c(end) - (e(end)-de(end)/2)) > 1000*eps(de(end));
                    warning(message('stats:hist3:InconsistentEdges'));
                    c = e(1:end-1) + de/2;
                end
