...
width = (1-left-right-hspace*(NumPlotsCOL-1))/NumPlotsCOL;
height = (1-top-bottom-vspace*(NumPlotsROW-1))/NumPlotsROW;

across = [width+hspace,0,0,0]';
down = -[0,height+vspace,0,0]';

pos = zeros(4,NumPlotsCOL,NumPlotsROW);

for d1=1:NumPlotsCOL
  for d2=1:NumPlotsROW
    pos(:,d1,d2) = [left, 1-top-height,width,height]' ...
                   + (d1-1)*across+(d2-1)*down;
  end
end

pos = reshape(pos,[4,NumPlotsROW*NumPlotsCOL]);
